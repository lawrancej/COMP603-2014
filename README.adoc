...
=== First sets, predict sets, follow sets
...
Predict set:: if the nonterminal derives the empty string, the first set union follow set, otherwise, first set.

=== LL(k) grammars

A context-free grammar in which the predict sets for each nonterminal are disjoint.
