...
* Implement some simple language, like say, http://en.wikipedia.org/wiki/Cool_(programming_language)[Cool].
* Something compiler-related that dovetails nicely with Senior project.

These ideas are also welcome:

* A parser combinator library to target multiple parsing backends (derivative, shift-reduce, or recursive descent parsing).

These projects build upon tools like clang, JDT, Python's ast module to do work:
...
* Something like http://en.wikipedia.org/wiki/QuickCheck[Quickcheck], or something that can generate characterization tests automatically.
