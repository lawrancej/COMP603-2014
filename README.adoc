...
echo "deadbeef" | sed -e 's/^\([a-f][a-f]*\)$/Hex: \1/'
echo "deadhorse" | sed -e 's/^\([a-f][a-f]*\)$/Hex: \1/'
----

With somebody sitting nearby, read the commands carefully.

. How do you think it works?
. What do you think +\w+ means?
. What do +\1+, +\2+ and +\3+ mean?
. *Challenge*: Can you write a sed command to match only identifiers in, say, C/C++ or Java?
. *Challenge*: Can you write finite state machines that correspond to the regular expressions above?

=== Regexes, State Machines and Derivatives, Oh My!

Let's talk about finite state machines and derivatives
