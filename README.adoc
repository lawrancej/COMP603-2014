...
git mergetool             # Use KDiff3 to merge my stuff in.
----
...
Empty String:: Match the empty string (+""+).
Symbol:: Match a single character (e.g., +a+).
Sequence:: Match one regular expression followed by one after another (e.g., if +a+ and +b+ are regular expressions, +ab+ matches +a+ followed by +b+).
Alternation:: Match either one regular expression or another (e.g., if +a+ and +b+ are regular expressions, +a|b+ matches +a+ or +b+).
Kleene Star:: Match a regular expression zero or more times (e.g., if +a+ is a regular expression, +a*+ matches +a+ zero or more times).
